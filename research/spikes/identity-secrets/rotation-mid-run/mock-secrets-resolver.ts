// Cat VIII spike #2 — shared mock SecretsResolver fixture (rotation-aware).
// Forked from spike #1 mock; same Result<T, E>, same SecretsError envelope,
// same name regex, same error codes. Adds two affordances:
//   - per-secret read counter + current-version tag (stats(name))
//   - invalidate(name) — bumps version + rotates value
// Caching is NOT done in the resolver; it always returns the current value.
// Caching is the adapter's problem (the whole point of this spike).
...
// --- Stats shape (spike #2 affordance) ---
...
interface Entry {
  base: string; // initial value, used to mint rotated values
  value: string;
  version: number;
  reads: number;
}
...
// "two runs in the same process" comparisons across variants.
...
// honest). Exported for backward-compat with spike-#1-shaped imports.

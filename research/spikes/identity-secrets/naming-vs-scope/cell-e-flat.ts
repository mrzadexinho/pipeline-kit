// Cat VIII spike #5 leg-2 sub-(1) — CELL (e): flat hyphenated naming.
// Source: secrets.resolve('apify-token'); Store: secrets.resolve('supabase-service-role').
...
function statsLine(label: string, name: string, s: Result<SecretStats, SecretsError>): string {
  if (s.error !== null) return `[cell-e] ${label} ${name} stats ERR ${s.error.code}`;
  return `[cell-e] ${label} ${name} reads=${s.data.reads} current_version=v${s.data.current_version}`;
}

// --- Source(apify) factory — FLAT naming ---
async function createApifySource(
  ...
  const tokenResult = await secrets.resolve('apify-token'); // factory-time site (FLAT)
  ...
  return ok({
    id: 'pk_src_apify_cell_e',
    async *iter(ctx) {
      const atomIds = ['pk_atom_CELL_E_01', 'pk_atom_CELL_E_02'];
...
// --- Cell driver ---
export async function runCellE(): Promise<boolean> {
  console.log('### CELL (e) — flat hyphenated naming, two-stage two-secret pipeline ###');
  ...
  if (storeBuilt.error !== null) {
    console.log(`[cell-e] store factory ERR ${storeBuilt.error.code}: ${storeBuilt.error.message}`);
    return false;
  }
  ...
  for await (const result of sourceBuilt.data.iter(ctx)) {
    ...
    if (result.error !== null) {
      ...
      console.log(`[cell-e] atom ${i} source ERR ${code}: ${msg}`);
      return false;
    }
    const atom = result.data;
    console.log(
      `[cell-e] atom ${i} source OK id=${atom.id} call_id=${atom.metadata.call_id_seen as string}`,
    );
    const put = await storeBuilt.data.put(atom, storeCtx);
    if (put.error !== null) {
      console.log(`[cell-e] atom ${i} store ERR ${put.error.code}: ${put.error.message}`);
      return false;
    }
